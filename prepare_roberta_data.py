...
tokenizer = AlbertTokenizer.from_pretrained(args.input_path + "/albert.large")
...
if args.tag_mspan:
    ...
    for dm in data_mode:
        ...
        print("Save data to {}.".format(os.path.join(args.output_dir, "tmspan_cached_albert_{}.pkl".format(dm))))
        with open(os.path.join(args.output_dir, "tmspan_cached_albert_{}.pkl".format(dm)), "wb") as f:
            pickle.dump(data, f)
    ...
    for dm in data_mode:
        dpath = os.path.join(args.input_path, data_format.format(dm))
        data = train_reader._read(dpath)
        print("Save data to {}.".format(os.path.join(args.output_dir, "cached_albert_{}.pkl".format(dm))))
        with open(os.path.join(args.output_dir, "cached_albert_{}.pkl".format(dm)), "wb") as f:
            pickle.dump(data, f)
